...
For a public GitHub version of this project, a practical setup is:

- keep the code in `R/`
- keep the final paper source file
- optionally keep the final PDF paper for easy viewing
- keep small final tables and figures in `tables/` if you want the repository to be easy to browse
- avoid versioning very large raw data files if they can be downloaded separately
- avoid versioning large intermediate files unless they are needed for a lightweight replication path

In other words, the most important public-facing pieces are the code, documentation, and final paper outputs. Large raw files and bulky intermediate data can be excluded if the README explains how to reconstruct them.
